...
static void on_realize(GtkGLArea *area) {
	...
	int i;
	for(i = 0; i enemy.active = malloc(enemy.num_enemies * sizeof(gboolean));
...
static gboolean on_idle(gpointer data) {
	...
	if(do_switch) {
		...
		for(i = 0; i float x_dif, y_dif, hypo, radius;
